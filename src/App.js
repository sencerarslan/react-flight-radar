...
export default function App({ sizeScale = 25, onDataLoad, mapStyle = MAP_STYLE }) {
  ...
  const [selectedAircraft, setSelectedAircraft] = useState(null); // Yeni durum
  ...
  const handleAircraftClick = (info, event) => {
    if (info && info.object) {
      setSelectedAircraft(info.object[DATA_INDEX.UNIQUE_ID]); // Uçağın kimliğini seçin
    }
  };
  ...
  const layer =
    ...
    new ScenegraphLayer({
      ...
      onClick: handleAircraftClick // Uçağa tıklama olayını dinle
    });
